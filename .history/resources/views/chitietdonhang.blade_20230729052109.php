            ...
            <div class="row">
                ...
                <div class="col-md-12 form-group">
                    ...
                    <input class="form-control" readonly   name="consingnee_name" type="text"
                        value="{{$order_detail['status_payment']consingnee_name}}">
                </div>
                <div class="col-md-12 form-group">
                    <label>Email</label>
                    <input class="form-control" readonly   name="consingnee_email" type="email"
                        value="{{$order_detail['status_payment']consingnee_email}}">
                </div>
                <div class="col-md-12 form-group">
                    <label>Phone</label>
                    <input class="form-control" readonly   name="consingnee_phone" type="number"
                        value="{{$order_detail['status_payment']consingnee_phone}}">
                </div>
                <div class="col-md-12 form-group">
                    <label>Address</label>
                    <input class="form-control" readonly   name="consingnee_address" type="text"
                        value="{{$order_detail['status_payment']consingnee_address}}">
                </div>
         ...
         <tr>
            ...
            <th>Mã giảm giá</th>
                ...
                {{-- @foreach($order_detail as $data) --}}
                 ...
                 <th>Không sử dụng</th>
                    @else
                    <th>{{$order_detail['coupon_code']}}</th>
                    @endif
                {{-- @endforeach --}}
            ...
            {{-- @foreach($order_detail as $data) --}}
            <th>{{number_format($order_detail['total'], 0, '.', '.') . ' VNĐ'}}</th>
            {{-- @endforeach --}}
        </table>
