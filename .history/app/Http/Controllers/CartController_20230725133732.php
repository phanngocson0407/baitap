...
class CartController extends Controller
{
    ...
    public function check_coupon(Request $request){
        ...
        ->first();
        // dd($check_quanti->total);
        // $count_order = DB::table('order')
        // ->select('coupon_code', DB::raw('count(*) as total'))
        // ->groupBy('coupon_code')
        // ->whereNotNull('order.coupon_code')
        // ->get();
        // $count_coupon = DB::table('coupon')
        // ->select('coupon_code','coupon_quanti')
        // ->get();
        //  dd($count_coupon);
        if($check_quanti_order->total < $check_quanti_coupon){
            
        }else{
            Session::forget('coupon'); 
            return redirect()->back()->with('error','Đã hết mã giảm ');
        }
